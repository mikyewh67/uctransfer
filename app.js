(() => {
  ...
  function calculateUnitsAndGpa() {
    ...
    allCourses.forEach((course) => {
      ...
      const courseState = getCourseState(course.id);
      if (courseState.done) completedUnits += course.units;
      else if (isCourseInProgress(course)) currentUnits += course.units;
      else futureUnits += course.units;
    ...
    const totalPlanUnits = completedUnits + currentUnits + futureUnits;
    return {
      completedUnits,
      currentUnits,
      futureUnits,
      activeUnits: completedUnits + currentUnits,
      plannedUnits: futureUnits,
      totalPlanUnits,
      gradedUnits,
      gradePoints,
      gpa
  ...
  function buildCalGetcCoverage() {
    ...
    const mapped = rows.filter((row) => row.courses.length > 0).length;
    const completed = rows.filter((row) => row.status === "covered").length;
    return {
      rows,
      mapped,
      completed,
      covered: completed,
      planComplete: mapped === DATA.calGetcAreas.length
    };
  }
  ...
  function renderToday() {
    ...
    const unitPlanMet = metrics.totalPlanUnits >= DATA.profile.unitGoal;
    const gePlanMet = coverage.planComplete;
    const planCloses = unitPlanMet && gePlanMet;
    const unitDifference = metrics.totalPlanUnits - DATA.profile.unitGoal;

    return `
      <section class="screen-header compact-header">
      ...
      <section class="plan-status-card section-gap" aria-labelledby="planStatusTitle">
        <div class="status-card-header">
          <div>
            <p class="eyebrow">TRANSFER PLAN</p>
            <h2 class="status-headline" id="planStatusTitle">${planCloses ? "The current plan reaches both requirements." : "The current plan still has a gap."}</h2>
          </div>
          <span class="status-chip${planCloses ? " ready" : ""}">${planCloses ? "PLAN CLOSES" : "ACTION NEEDED"}</span>
        </div>

        <button type="button" class="status-row" data-action="open-tab" data-tab="plan">
          <span class="status-value">${metrics.totalPlanUnits} / ${DATA.profile.unitGoal}</span>
          <span class="status-copy"><strong>Transferable units planned</strong><small>${metrics.completedUnits} complete · ${metrics.currentUnits} current · ${metrics.futureUnits} future${unitPlanMet ? ` · ${unitDifference} unit cushion` : ` · ${Math.abs(unitDifference)} more needed`}</small></span>
          <span class="row-arrow" aria-hidden="true"></span>
        </button>

        <button type="button" class="status-row" data-action="open-tab" data-tab="ge">
          <span class="status-value">${coverage.mapped} / ${DATA.calGetcAreas.length}</span>
          <span class="status-copy"><strong>Cal-GETC areas assigned</strong><small>${coverage.completed} complete · ${coverage.mapped - coverage.completed} still in the plan</small></span>
          <span class="row-arrow" aria-hidden="true"></span>
        </button>

        <p class="status-explainer">You do not have 60 completed credits yet. Your listed courses total ${metrics.totalPlanUnits} transferable units, and the GE courses already assign all ${DATA.calGetcAreas.length} Cal-GETC areas.</p>
      </section>
  ...
  function renderPlan() {
    ...
    const capMessage = metrics.totalPlanUnits > DATA.profile.unitSoftCap
      ...
      : `${metrics.totalPlanUnits} transferable units are listed. UC applies a soft transfer-credit cap at ${DATA.profile.unitSoftCap} community-college semester units.`;

    return `
      <section class="screen-header compact-header">
        <p class="eyebrow">TWO-YEAR RECORD</p>
        <h1 class="screen-title">Plan</h1>
        <p class="screen-intro">Your completed record and your future course plan are shown separately.</p>
      </section>

      <section class="plan-result-card" aria-labelledby="planResultTitle">
        <div class="status-card-header">
          <div>
            <p class="eyebrow">CURRENT RESULT</p>
            <h2 class="status-headline" id="planResultTitle">${planCloses ? "Yes. This course plan reaches 60 units and maps every GE area." : "No. The current course plan still has an unmet requirement."}</h2>
          </div>
          <span class="status-chip${planCloses ? " ready" : ""}">${planCloses ? "ON PLAN" : "GAP"}</span>
        </div>
        <div class="plan-result-lines">
          <div><span>Planned transferable units</span><strong>${metrics.totalPlanUnits} / ${DATA.profile.unitGoal}</strong></div>
          <div><span>Cal-GETC areas assigned</span><strong>${coverage.mapped} / ${DATA.calGetcAreas.length}</strong></div>
          <div><span>Actually completed</span><strong>${metrics.completedUnits} units</strong></div>
        </div>
        <p class="status-explainer">The ${metrics.totalPlanUnits}-unit number includes future classes. Only ${metrics.completedUnits} units are completed on the current record.</p>
      </section>

      <div class="plan-layout section-gap" id="planSpine" aria-label="The Spine transfer plan">
      ...
      <div class="plan-footer mono${metrics.completedUnits >= DATA.profile.unitGoal ? " goal-met" : ""}">
        <span id="unitCounter">${metrics.completedUnits}</span> / ${DATA.profile.unitGoal} COMPLETE · ${metrics.totalPlanUnits} PLANNED · GPA ${gpaDisplay}
      </div>
    `;
  }
  ...
  function renderGe() {
    ...
    const offset = circumference * (1 - coverage.mapped / DATA.calGetcAreas.length);
    const statusLabels = {
      covered: "Complete",
      "in progress": "Current",
      planned: "Planned",
    ...
    return `
      <section class="screen-header compact-header">
        ...
        <p class="screen-intro">This separates courses already chosen in your plan from courses you have actually completed.</p>
      </section>

      <section class="ge-answer-card" aria-labelledby="geAnswerTitle">
        <p class="eyebrow">DIRECT ANSWER</p>
        <h2 class="status-headline" id="geAnswerTitle">${coverage.planComplete ? "All 11 Cal-GETC areas already have a course assigned." : `${DATA.calGetcAreas.length - coverage.mapped} Cal-GETC area${DATA.calGetcAreas.length - coverage.mapped === 1 ? "" : "s"} still need a course.`}</h2>
        <p class="status-explainer">Assigned does not mean completed. You still need a C or better in each qualifying course and must request certification.</p>
      </section>

      <details class="accordion section-gap" open>
        <summary>
          <span class="summary-text"><span class="summary-title">Your GE choices</span><span class="summary-meta">${coverage.mapped} assigned · ${coverage.completed} completed</span></span>
          <span class="chevron" aria-hidden="true"></span>
        </summary>
        <div class="accordion-body"><div class="accordion-inner"><div class="accordion-content">
          <div class="ring-layout">
            <div class="coverage-ring" role="img" aria-label="${coverage.mapped} of 11 Cal-GETC areas assigned a course">
              ...
              <div class="ring-text"><strong>${coverage.mapped} / 11</strong><span>assigned</span></div>
            </div>
            <div>
              <h3>${coverage.completed} of 11 completed</h3>
              <p class="muted">The ring shows whether the two-year plan has a valid course for every area. The smaller number shows what is already passed.</p>
            </div>
          </div>
            ...
            ${coverage.rows.map((row) => {
              ...
              return `
                <li class="coverage-row">
                  ...
                  <div><p class="row-title">${escapeHtml(row.name)}</p><p class="row-detail">${escapeHtml(statusLabels[row.status] || row.status)}</p></div>
      ...
      <details class="accordion section-gap">
        <summary>
          <span class="summary-text"><span class="summary-title">Other GE options</span><span class="summary-meta">S through D</span></span>
          <span class="chevron" aria-hidden="true"></span>
        </summary>
        <div class="accordion-body"><div class="accordion-inner"><div class="accordion-content">
          <p class="muted">These are substitutes and backups. Your main plan already uses the courses shown above.</p>
  ...
  function switchTab(tab) {
    if (!tab || !["today", "plan", "ge", "apply", "playbook"].includes(tab) || tab === state.activeTab) return;
